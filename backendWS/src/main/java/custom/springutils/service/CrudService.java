...
import custom.springutils.util.map.MapUtil;
import custom.springutils.search.FieldOperator;
import custom.springutils.search.SearchOperator;
    ...
    protected ListResponse search(Object filter, Map<String, Object> criteria, Integer page, Class<?> source, Object order) throws Exception {
        ...
        for (String key : criteria.keySet()) {
            ...
            if (op.operator == SearchOperator.isnotnull || op.operator == SearchOperator.isnull) {
                ...
                continue;
            }else if(op.operator == SearchOperator.like || op.operator == SearchOperator.ilike){
                params.add("%" + val + "%");
            }else {
                params.add(val);
            }
