...
export const SearchCard = ({searchFormData, pagination}) => {
    function search() {
        ...
        for (let i = 0; i < searchInputs.length; i++) {
            if(searchInputs[i].value===undefined || searchInputs[i].value===null || searchInputs[i].value==="")
                continue
            query+="&&"+searchInputs[i].id+"="+searchInputs[i].value
        }
    ...
    return (
        <>
            {(pagination.length!==0 && searchFormData!==undefined) && <DynamicForm searchFormData={searchFormData===undefined?[]:searchFormData} search={search} btnValue={"Search"}/>}
        </>
    );
}